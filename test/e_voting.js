...
contract("e_voting", function(accounts) {
    ...
    it("initializes with an address", function() {
        return e_voting.deployed().then(function(instance) {
    ...
    it("it initializes the candidates with the correct values", function() {
        return e_voting.deployed().then(function(instance) {
            ...
            assert.equal(candidate[1], "MR. LEWIS HAMILTON", "contains the correct name");
            assert.equal(candidate[2], 0, "contains the correct votes count");
            return electionInstance.candidates(2);
        }).then(function(candidate) {
            assert.equal(candidate[0], 2, "contains the correct id");
            assert.equal(candidate[1], "MRS. ANGELA MAX", "contains the correct name");
            assert.equal(candidate[2], 0, "contains the correct votes count");
            return electionInstance.candidates(3);
        }).then(function(candidate){
            assert.equal(candidate[0], 3, "contains the correct id");
            assert.equal(candidate[1], "MR. SEBASTIAN VETTEL", "contains the correct name");
            assert.equal(candidate[2], 0, "contains the correct votes count");
        });
    });
